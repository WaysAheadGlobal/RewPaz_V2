...
class EmployeeController extends Controller
{
        ...
        if($user >0){
            ...
            if (Hash::check($user_given_otp, $verifiedUser->otp))
            {
                ...
                if(auth()->user()->status == 1){
                    ...
                    $notification->msg = 'Your employee is recently  login !!';
    ...
    public function tokens_raise(Request $request){
       ...
       // dd($result);
        ...
        $notification->msg = 'Your employee has raise a request for token.';
        //$notification->row = $insertedUserId;
        $notification->userType = 'Employee';
        $notification->uuid = Str::uuid();
        $notification->save();
    ...
    public function save_suggestion(Request $request)
    {
       // dd($request->all());
        ...
        $data = array(
            ...
            'mental_wellness' => implode(',',$request->mental_wellness),
            'physical_wellness' => implode(',',$request->physical_wellness),
            'household_service' => implode(',',$request->household_service),
