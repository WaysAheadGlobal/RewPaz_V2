<div class="anything">
 @include('livewire.add-journals-modal')
 @include('livewire.view-notes-modal')
        ...
        <div class="time_slots">
          ...
          @if($journals)
            ...
            <li>
               ...
               @if($timeMatched)
                  <a href="#" data-bs-toggle="modal" data-bs-target="#modalAddNote" wire:click="openEditNote('{{$note->uid}}')" style="background: #006ac4;padding: 5px 15px;border-radius: 5px;color: #fff;">View/Edit Journal</a>
                  @else
                  <a href="#" data-bs-toggle="modal" data-bs-target="#modalAddNote" wire:click="openNote('{{$dataTime['24hr']}}', '')" style="background: #accdff;padding: 5px 15px;border-radius: 5px;color: #fff;">Add Journal</a>
               @endif
            </li>
          @endif
          @else
          ...
          @if($notes)
            ...
            <li>
               <span> {{$dataTime['12hr']}}</span>
               @if($timeMatched)
                  <a href="#" data-bs-toggle="modal" data-bs-target="#modalViewNote" wire:click="openViewNote('{{$note->uid}}')" style="background: #006ac4;padding: 5px 15px;border-radius: 5px;color: #fff;">View Note</a>
                  @else
                  <i style="opacity:0.4; text-transform:none; font-size:10px">No Note Added</i>
               @endif
            </li>
          @endif
          @endif
